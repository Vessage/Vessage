...
-(void)submit
{
    ...
    else
    {
        [[NSNotificationCenter defaultCenter] addObserver:self
                                                 selector:@selector(OnValidateMobileCodeResult:)//接收消息方法
                                                     name:@"OnValidateMobileCodeResult"//消息识别名称
                                                   object:nil];
        self.verificationCodeResult (SMSUIResponseStateSelfVerify,_phone,_areaCode,self.verifyCodeField.text, verifyError);
    }
}

//实现方法
-(void)OnValidateMobileCodeResult:(NSNotification *)a
{
    [[NSNotificationCenter defaultCenter] removeObserver:self name:@"OnValidateMobileCodeResult" object:nil];
    NSError *error = a.object;
    if (!error) {
...
-(void)alertView:(UIAlertView *)alertView clickedButtonAtIndex:(NSInteger)buttonIndex
{
    ...
    {
        self.verificationCodeResult (SMSUIResponseStateSuccess,_phone,_areaCode,self.verifyCodeField.text, verifyError);
        [verifyViewController.timer2 invalidate];
        [verifyViewController.timer1 invalidate];
