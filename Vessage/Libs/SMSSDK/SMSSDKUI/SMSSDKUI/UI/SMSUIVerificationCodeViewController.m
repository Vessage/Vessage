...
- (void)show
{
    ...
    registerViewBySMS.verificationCodeResult = ^(enum SMSUIResponseState state,NSString *phoneNumber,NSString *zone,NSString *code,NSError *error){
        
        if (state == SMSUIResponseStateCancel) {
            
            if (verificationCodeVC.verificationCodeResult) {
                
                verificationCodeVC.verificationCodeResult (SMSUIResponseStateCancel ,phoneNumber,zone,code,error);
            ...
            if (verificationCodeVC.verificationCodeResult) {
                ...
                verificationCodeVC.verificationCodeResult (SMSUIResponseStateSuccess,phoneNumber,zone,code,error);
            }
            ...
            if (verificationCodeVC.verificationCodeResult) {
                ...
                verificationCodeVC.verificationCodeResult (SMSUIResponseStateFail,phoneNumber,zone,code,error);
                
            }
        } else if (state == SMSUIResponseStateSelfVerify ) {
            
            if (verificationCodeVC.verificationCodeResult) {
                
                verificationCodeVC.verificationCodeResult (SMSUIResponseStateSelfVerify,phoneNumber,zone,code,error);
                
            }
        }
        
        if (state != SMSUIResponseStateSelfVerify) {
