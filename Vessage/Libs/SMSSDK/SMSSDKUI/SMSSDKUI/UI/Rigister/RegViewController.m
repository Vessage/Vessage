...
-(void)clickLeftButton
{
    if (self.verificationCodeResult) {
        ...
        self.verificationCodeResult (SMSUIResponseStateCancel,self.telField.text,str2, nil,nil);
    }
}
...
//    NSBundle *bundle = [[NSBundle alloc] initWithPath:filePath];
    ...
    if (!error)
    {
        ...
        //发送验证码成功，进行回调
        verify.verificationCodeResult = ^(enum SMSUIResponseState state,NSString *phoneNumber,NSString *zone,NSString *code,NSError *error){
            
            if (!error) {
                
                if (state == SMSUIResponseStateSuccess || state == SMSUIResponseStateSelfVerify) {
                    
                    self.verificationCodeResult (state,phoneNumber,zone,code, error);
                }
